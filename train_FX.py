...
def main():
    ...
    csv_files = glob.glob(f"labeled_data/*_{args.mode}_scaled_FX.csv")
    if not csv_files:
    ...
    model_path = os.path.join(model_dir, f"model_{args.mode}_FX.pth") # .pth に統一
    ...
    drive_model_path = os.path.join(drive_model_dir,f"model_{args.mode}_FX.pth")
    ...
    for epoch in range(start_epoch,args.epochs):
       ...
       0.5 * torch.exp(-log_var_r) * loss_r + 0.5 * log_var_r
        ...
        if avg_val_total < best_val_total_loss:
            ...
            renewed = 0
            path = os.path.join(model_dir, f"best_total_model_{args.mode}_FX.pth")
        ...
        if avg_val_class < best_val_class_loss:
            ...
            renewed = 0
            path = os.path.join(model_dir, f"best_class_model_{args.mode}_FX.pth")
            torch.save(checkpoint_data,path)
        ...
        if avg_val_risk < best_val_risk_loss:
            ...
            renewed = 0
            path = os.path.join(model_dir, f"best_risk_model_{args.mode}_FX.pth")
            torch.save(checkpoint_data, path)
        ...
        if drive_service:
            models_to_upload = [
                os.path.join(model_dir, f"best_total_model_{args.mode}_FX.pth"),
                os.path.join(model_dir, f"best_class_model_{args.mode}_FX.pth"),
                os.path.join(model_dir, f"best_risk_model_{args.mode}_FX.pth"),
                os.path.join(model_dir, f"model_{args.mode}_FX.pth")
            ]
