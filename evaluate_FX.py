...
def main():
    ...
    for csv_file in test_files:
        ...
        try:
            # データセットの作成
            dataset = FXMultimodalDataset(csv_file=csv_file, img_dir=img_dir, seq_length=15)
    ...
    model_variants = {
        "Best TOTAL Model (Class + Risk balanced)": f"best_total_model_{args.mode}_FX.pth",
        "Best CLASS Model (Accuracy optimized)": f"best_class_model_{args.mode}_FX.pth",
        "Best RISK Model (MAE optimized)": f"best_risk_model_{args.mode}_FX.pth",
        "Final Epoch Model": f"model_{args.mode}_FX.pth"
    }
    ...
    if os.path.exists("/content/drive/MyDrive"):
        ...
        drive_best_model_dest = os.path.join(drive_model_dir, f"final_best_model_{args.mode}_FX.pth")
